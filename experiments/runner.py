...
from .factories import AlgorithmFactory, EnvironmentFactory
...
class ExperimentRunner:
    ...
    def _create_components(self):
        ...
        algorithm = AlgorithmFactory.create(self.config, obs_space, act_space, device=device)
    ...
    def _integrate_with_tmrl(self):
        ...
        from tmrl.custom.custom_memories import MemoryTMFull, MemoryTMLidar
        ...
        if self.config.model_type == "mlp":
            ...
            memory_cls = MemoryTMFull  # type: ignore[assignment]

        # Create partial for environment
        env_cls = partial(EnvironmentFactory.create, self.config)  # type: ignore[arg-type]
        ...
        algorithm_partial = partial(AlgorithmFactory.create, self.config, obs_space, act_space, device)  # type: ignore[arg-type]
    ...
    def run(self, test_mode: bool = False):
        ...
        try:
            ...
            logging.info(
                f"Epochs: {trainer_instance.epochs}, Rounds: {trainer_instance.rounds}, Steps: {trainer_instance.steps}"
            )
